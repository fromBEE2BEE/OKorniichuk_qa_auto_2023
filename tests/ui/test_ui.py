...
def test_search_cargo_positive():
    ...
    # Знаходимо поле, в яке будемо вводити коректний номер накладної
    cargo_number_elem = driver.find_element(By.ID, "cargo_number")
    
    # Вводимо коректний номер накладної
    cargo_number_elem.send_keys('20450857971424')
    
    # Знаходимо кнопку 'submit'
    btn_elem = driver.find_element(By.CLASS_NAME, "search_cargo_form")
    
    # Емулюємо клік лівою кнопкою мишки
    btn_elem.click()
       
    # Перевіряємо, що назва сторінки така, яку ми очікуємо
    assert driver.title == 'Термінова і експрес доставка: транспортно-логістичні послуги в Києві та по всій Україні - служба доставки №1 «Нова пошта»'
